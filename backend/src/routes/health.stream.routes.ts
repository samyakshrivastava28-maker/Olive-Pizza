...
const gatherMetrics = async () => {
  ...
  try {
    ...
    try { const eq = await c.query("SELECT count(*) FROM emails WHERE status='pending'"); m.services.email.queueSize = parseInt(eq.rows[0].count,10)||0; m.services.email.status='operational'; } catch { m.services.email.status='degraded'; }
    try { const tk = await c.query('SELECT count(*) FROM fcm_tokens'); const nq = await c.query("SELECT count(*) FROM notifications WHERE status='pending'"); m.services.notifications.activeTokens=parseInt(tk.rows[0].count,10)||0; m.services.notifications.queued=parseInt(nq.rows[0].count,10)||0; m.services.notifications.status='operational'; } catch { m.services.notifications.status='degraded'; }
    c.release();
