...
export class MonthlyReportService {
   ...
   * Can be run automatically via cron or triggered manually from Owner Dashboard.
   */
  public async generateAndProcessReport(targetDate?: Date): Promise<ReportGenerationResult> {
    ...
    let driveLink = '';
    let driveFileId = '';
    const fileName = `OlivePizza_Report_${reportYear}_${(reportMonth).toString().padStart(2, '0')}.pdf`;
    ...
    // Primary Firestore location: reports (sub-document or document)
      ...
      const emailHtml = `
        <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 24px; border-radius: 12px; border: 1px solid #e2e8f0;">
          ...
          <div style="border-top: 1px solid #e2e8f0; padding-top: 15px; text-align: center; color: #94a3b8; font-size: 12px;">
            Olive Pizza Inc. • Automated Reporting System
          </div>
        </div>
      `;
  ...
  private async collectMetrics(
    ...
    // 1. Query Firestore Orders for month
    ...
    const activeDeliveryPartners = partnerDocs.docs.length || 3;
    const deliveryPartnerStats = partnerDocs.docs.slice(0, 5).map(doc => {
      const data = doc.data();
      return {
