...
client.connect()
  .then(() => client.query("SELECT status FROM notification_history ORDER BY created_at DESC LIMIT 1"))
  .then(res => { console.log('History Status:', res.rows[0]?.status); process.exit(0); })
