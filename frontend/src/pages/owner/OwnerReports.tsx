...
import { FileText, Download, ExternalLink, RefreshCw, Mail, Search, ShieldCheck, HardDrive, CheckCircle2, AlertTriangle, Cpu, TrendingUp, Layers, Activity } from "lucide-react";
...
export default function OwnerReports() {
  ...
  const [activeTab, setActiveTab] = useState<"reports" | "diagnostics">("reports");
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const snapshot = await getDocs(q).catch(() => getDocs(query(collection(db, "monthly_reports"), orderBy("generatedAt", "desc"))));
  ...
  const handleGenerateReport = async (year?: number, month?: number) => {
    setGenerating(true);
    const toastId = toast.loading("Queuing report generation task...");
    try {
      const token = await auth.currentUser?.getIdToken();
      ...
      if (data.success) {
        toast.success(data.message || "Report queued successfully!", { id: toastId });
        setTimeout(() => {
          fetchReports();
          fetchDiagnostics();
        }, 3000);
  ...
  const handleEmailAgain = async (docId: string) => {
    const toastId = toast.loading("Resending report email...");
    try {
      const token = await auth.currentUser?.getIdToken();
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this report record from Firestore?")) return;
    try {
      await deleteDoc(doc(db, "reports", id)).catch(() => deleteDoc(doc(db, "monthly_reports", id)));
  ...
  const filteredReports = reports.filter((r) => {
    const monthText = r.monthName || new Date(r.year, r.month - 1).toLocaleString("default", { month: "long" });
    const matchesSearch = monthText.toLowerCase().includes(searchTerm.toLowerCase()) || r.year?.toString().includes(searchTerm);
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-16">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900/80 p-6 rounded-3xl border border-slate-800 backdrop-blur-xl">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-black text-white">Monthly Reports & Intelligence</h1>
              <p className="text-slate-400 text-sm">
                Automated monthly PDF reports, Google Drive backups, and system diagnostics.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {generating ? "Queuing..." : "Generate Latest Report"}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-900/50 border border-slate-800 p-5 rounded-3xl flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Tracked Revenue</p>
        ...
        <div className="bg-slate-900/50 border border-slate-800 p-5 rounded-3xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-slate-200">olivepizzarjn@gmail.com</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-800">
        ...
        >
          <FileText className="w-4 h-4" /> Generated Reports ({filteredReports.length})
        </button>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-between items-center bg-slate-900/40 p-4 rounded-2xl border border-slate-800">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <select
                ...
                className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-orange-500"
              >
          ...
          <div className="bg-slate-900/60 rounded-3xl border border-slate-800 overflow-hidden shadow-xl">
            {loading ? (
              ...
              <div className="p-16 text-center space-y-4">
                ...
                <p className="text-slate-400 font-medium">No reports found matching your filter.</p>
                <button
                  onClick={() => handleGenerateReport()}
                      ...
                      return (
                        <tr key={report.id} className="hover:bg-slate-800/30 transition-colors">
                          <td className="p-4">
                            ...
                            <div className="text-xs text-slate-500 font-mono">Doc: {report.id}</div>
                          </td>
                          <td className="p-4 font-black text-green-400">
                            ₹{(report.totalRevenue || 0).toLocaleString("en-IN")}
                          </td>
                          <td className="p-4 font-bold text-slate-200">{report.totalOrders || 0}</td>
                          ...
                          <td className="p-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button
                                onClick={() => handleGenerateReport(report.year, report.month)}
