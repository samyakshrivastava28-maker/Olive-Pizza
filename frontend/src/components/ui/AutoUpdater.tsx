  ...
  useEffect(() => {
    const checkForUpdates = async () => {
      ...
      try {
        ...
        const res = await fetch("https://api.github.com/repos/samyakshrivastava28-maker/Olive-Pizza/releases/latest");
        if (!res.ok) return;
        ...
        if (asset) {
          ...
          if (githubVersionCode > currentVersionCode) {
            setDownloadUrl(asset.browser_download_url || '/api/github/download-apk');
